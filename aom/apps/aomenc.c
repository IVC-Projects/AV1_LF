...
int main(int argc, const char **argv_) {
  ...
  struct AvxInputContext input;
  struct AvxEncoderConfig global;//全局的配置变量
  ...
  input.framerate.numerator = 30;//帧率的分子
  input.framerate.denominator = 1;//帧率的分母，帧率为1/30
  input.only_i420 = 1;
  input.bit_depth = 0;
  ...
  switch (global.color_type) {//颜色类型，有4:2:0,4:2:2等等
  ...
  /* Check for unrecognized options 检查未知的选项*/
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    /* If the input file doesn't specify(指定) its w/h (raw files), try to get
    ...
    FOREACH_STREAM(stream, streams) {
      ...
      /* Automatically set the codec bit depth to match(适应) the input bit depth.
