...
/**\brief Image Descriptor (简要图像描述)*/
typedef struct aom_image {
  aom_img_fmt_t fmt;                 /**< Image Format 图像格式*/
  aom_color_primaries_t cp;          /**< CICP Color Primaries 颜色初始化*/
  aom_transfer_characteristics_t tc; /**< CICP Transfer Characteristics */
  aom_matrix_coefficients_t mc;      /**< CICP Matrix Coefficients 矩阵系数*/
  int monochrome;                    /**< Whether image is monochrome 图像是否是单色的（只有Y值）*/
  aom_chroma_sample_position_t csp;  /**< chroma sample position 色度值*/
  aom_color_range_t range;           /**< Color Range 颜色种类*/

  /* Image storage dimensions 存储图像的尺寸*/
  unsigned int w;         /**< Stored image width 存储图像的宽度*/
  unsigned int h;         /**< Stored image height 存储图像的高度*/
  unsigned int bit_depth; /**< Stored image bit-depth */

  /* Image display dimensions 图像展示尺寸*/
  ...
  /* Chroma subsampling info 色度的子采样*/
  unsigned int x_chroma_shift; /**< subsampling order, X */
  unsigned int y_chroma_shift; /**< subsampling order, Y */

/* Image data pointers. 图像数据指针*/
...
#define AOM_PLANE_ALPHA 3   /**< A (Transparency) plane */
  unsigned char *planes[4]; /**< pointer to the top left pixel for each plane 左上角第一个像素的数据指针*/
  int stride[4];            /**< stride between rows for each plane 每个平面的步长*/
  size_t sz;                /**< data size */
